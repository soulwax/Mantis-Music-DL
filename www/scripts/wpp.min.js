// File: httpdocs/scripts/wpp.min.js
...
const WordPressPopularPosts = (function () {
  ...
  const ajax = function (method, url, params, callback, additional_headers) {
    ...
    if (
      "object" == typeof additional_headers &&
      Object.keys(additional_headers).length
    )
  ...
  const theme = function (wpp_list) {
    ...
    let dummy_list_item_styles = getComputedStyle(
        dummy_list.querySelector("li")
      ),
      dummy_link_item_styles = getComputedStyle(
        dummy_list.querySelector("li a")
      );
    base_styles.innerHTML =
      ".wpp-list li {font-size: " + dummy_list_item_styles.fontSize + "}";
    base_styles.innerHTML +=
      ".wpp-list li a {color: " + dummy_link_item_styles.color + "}";
...
document.addEventListener("DOMContentLoaded", function () {
  if (!Object.keys(wpp_params).length) return;
  const widget_placeholders = document.querySelectorAll(
  ...
  if (sr.length)
    for (let s = 0; s < sr.length; s++) WordPressPopularPosts.theme(sr[s]);
  function fetchWidget(widget_placeholder) {
    ...
    let json_tag = widget_placeholder.parentNode.querySelector(
  ...
  function renderWidget(response, widget_placeholder) {
    widget_placeholder.insertAdjacentHTML(
      "afterend",
      JSON.parse(response).widget
    );
